...
var getColors = function(img, x, y, p){
	...
	var id = out.red / SCOPE + p / 640;

	out.full = out.red + out.green + out.blue;

	out.id = ""+ Math.floor(id);
...
co(function*(){
	...
	while(item.done === false){
		...
		for(var i=0; i<numImgs; i++){
			...
			pixel.dist = Math.abs(pixel.full - next.full);
			pixel.next = next.id;
	...
	for(var i=0; i<numImgs; i++){
		...
		pixel.dist = pixel.blue - next.blue;
		pixel.next = next.id;
