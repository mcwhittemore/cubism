...
co(function*(){
	...
	for(var i=0; i<imgLength; i++){
		var here = avgs[i];
		var range = Math.floor(here.green);
		...
		pixels.set(avgs[i].x, avgs[i].y, 0, midAvg.red);
		pixels.set(avgs[i].x, avgs[i].y, 1, midAvg.green);
		pixels.set(avgs[i].x, avgs[i].y, 2, midAvg.blue);
	}
