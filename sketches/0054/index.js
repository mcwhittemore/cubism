...
co(function*(){
	...
	for(var b=1; b<numImgs; b++){
		...
		while(next.done === false){
			...
			var red = Math.floor(img.get(x, y, 0) / 16);
			var green = Math.floor(img.get(x, y, 1) / 16);
			var blue = Math.floor(img.get(x, y, 2) / 16);

			current.push((red << 8 | green << 4 | blue) * SHIFT_WEIGHT);
	...
	while(next.done === false){
		...
		var seed = (red << 8 | green << 4 | blue) * SHIFT_WEIGHT;
		get.push(seed);
		...
		if(get.length == BLOCK_SIZE){
			...
			for(var i=0; i<data.length; i++){
				...
				var red = val >> 8;
				var green = val >> 4; // how do unshift
