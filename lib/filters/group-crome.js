...
var getColors = function(img, x, y, breakDown){
...
var findBlock = function(ref, blocksByRef, links){
	if(links[ref]){
		return findBlock(links[ref], blocksByRef, links);
	}
	else{
		return blocksByRef[ref];
	}
}
...
module.exports = function(imgs, numColors, blockSize, blockStringer, outerMin, innerMin){
	...
	while(refs.length > numColors){
		...
		for(var i=1; i<refs.length && i<numColors*2; i++){
			var a = refs[i];
	...
	for(var i=0; i<imgs.length; i++){
		...
		while(next.done === false){
			...
			if(xys.length===blockSize){
				var ref = i+"-"+loc;
				var block = findBlock(ref, blocksByImgAndLoc, links);
